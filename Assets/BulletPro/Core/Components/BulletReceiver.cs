...
namespace BulletPro
{
	...
	public class BulletReceiver : MonoBehaviour
	{
		...
		private bool awakeHappened = false;
		#endif
		...
		{
#if UNITY_EDITOR
			if (!awakeHappened && startingChildren != null && startingChildren.Length > 0)
				Debug.LogWarning("BulletPro Warning: it seems you're trying to set your Composite Receiver's .enabled property during Awake.\nThis will cause problem as its children aren't bound to it yet. Please wait one frame before doing this!");
#endif
