...
namespace BulletPro.EditorScripts
{
	...
	public class BulletParamInspector : EmissionParamsInspector
	{
			...
			if (isVisible.boolValue)
				if (renderMode.enumValueIndex == 0)
				{
					...
					if (!DynamicParameterUtility.GetBool(animated))
						spriteExists = DynamicParameterUtility.GetMostLikelyObject(sprite) != null;
					else
					{
						if (sprites.arraySize > 0) spriteExists = DynamicParameterUtility.GetFixedObject(sprites.GetArrayElementAtIndex(0)) != null;
						if (!spriteExists) spriteExists = DynamicParameterUtility.GetMostLikelyObject(sprite) != null;
					}
		...
		{
			Object defaultSprite = DynamicParameterUtility.GetMostLikelyObject(sprite);
			...
			if (canBeAnimated)
			{
				...
				{
					SerializedProperty spriteProp = sprites.GetArrayElementAtIndex(0);
					firstAnimSprite = DynamicParameterUtility.GetMostLikelyObject(spriteProp);
				}

				spriteToUse = (firstAnimSprite ?? defaultSprite) as Sprite;
			}
			else spriteToUse = defaultSprite as Sprite;
			...
			if (colliders.arraySize > 0)
				for (int i = 0; i < colliders.arraySize; i++)
				{
					...
					else /* */ if (colTypes[i] == (int)BulletColliderType.Circle)
					{
						...
						if (dist2 < rad2)
						{
							if (dist2 > (rad2 * 0.6f))//- 1.5f * tex.width))
								cur = AlphaBlend(gizmoColor, cur);
							else
